    ...
    <div>
        ...
        >
            ...
            <el-form-item
                label="明细分类："
                prop="type"
                v-if="formData.classify == '3' || formData.classify == '4'"
            >
                <el-radio-group v-model="formData.type">
                    <el-radio v-if="formData.classify == 3" label="1"
                        >研发库存</el-radio
                    >
                    <el-radio v-if="formData.classify == 3" label="2"
                        >产品库存</el-radio
                    >

                    <el-radio v-if="formData.classify == 4" label="1"
                        >工具采购</el-radio
                    >
                    <el-radio v-if="formData.classify == 4" label="2"
                        >办公用品</el-radio
                    >
                    <el-radio v-if="formData.classify == 4" label="3"
                        >办公家具</el-radio
                    >
                    <el-radio v-if="formData.classify == 4" label="4"
                        >税款</el-radio
                    >
                </el-radio-group>
            </el-form-item>
            ...
            >
                ...
                >
                    <el-option
                        v-for="(item, index) in itemOptions1"
                        :value="item.value"
                        :key="index"
                        :label="item.label"
                    >
            ...
            >
                ...
                >
                    <el-option
                        v-for="(item, index) in itemOptions2"
                        :value="item.value"
                        :key="index"
                        :label="item.label"
                    >
            ...
            <div v-if="formData.isPublic == '1'">
                <el-form-item label="金额：" prop="isLarge">
                    ...
                    <br />
                    <el-tag
                        v-if="formData.isLarge == '0'"
                        size="mini"
                        type="danger"
                        >打款日期审批通过后的下一个周一、周三、周五</el-tag
                    >
                    <el-tag
                        v-if="formData.isLarge == '1'"
                        size="mini"
                        type="danger"
                        >打款日期审批通过后的（加7天）下一个周一、周三、周五</el-tag
                    >
                </el-form-item>
            </div>
...
export default {
    ...
    created() {
        ...
        const editExpenseInfo = JSON.parse(
            window.localStorage.getItem("editExpenseInfo")
        );
        if (editExpenseInfo) {
    ...
    methods: {
        ...
        handleRemove2(data) {
            var serverUrl = `${window.location.origin}/apply/expense/`;
            const imgUrl = data.url.replace(serverUrl, "");
            this.formData.content = this.formData.contract.replace(
                imgUrl + ",",
                ""
            );
        ...
        getStandardApplyInfo() {
            getStandardApplyInfoById(7).then(response => {
                if (response.data.data) {
                    ...
                    const expenseStandardApply = JSON.parse(
                        window.localStorage.getItem("expenseStandardApply")
                    );
