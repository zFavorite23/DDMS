        ...
        >
            ...
            <el-form-item label="甲方信息：">
                <el-select
                    ...
                    placeholder="请选择"
                    @change="selectFirstParty"
                >
                    <el-option
                        v-for="(item, index) in firstPartyOptions"
                        :value="item.firstPartyId"
                        :key="index"
                        :label="item.name"
                    >
                    </el-option>
            ...
            <el-form-item label="开票百分比（%）：" prop="proportion">
                <el-input
                    ...
                    placeholder="请填写开票百分比"
                    maxlength="10"
                    show-word-limit
                ></el-input>
            </el-form-item>
            <el-form-item label="开票金额（元）：" prop="priceYuan">
                <el-input
                    type="number"
            ...
            <el-form-item
                label="产品清单："
                prop="detailedList"
                v-if="formData.content == 22 || formData.content == 23"
            >
                <el-upload
                    ref="upload"
                    :action="uploadUrl"
...
export default {
    ...
    created() {
        ...
        const editReceiptInfo = JSON.parse(
            window.localStorage.getItem("editReceiptInfo")
        );
        if (editReceiptInfo) {
        ...
        getStandardApplyInfo() {
            getStandardApplyInfoById(8).then(response => {
                if (response.data.data) {
                    ...
                    const receiptStandardApply = JSON.parse(
                        window.localStorage.getItem("receiptStandardApply")
                    );
