<template>
    <div>
        ...
        >
            ...
            <el-form-item
                label="请假时间："
                v-if="formData.type == '9' || formData.type == '10'"
            >
                <el-date-picker
                    v-model="formData.start"
...
export default {
    ...
    created() {
        ...
        const editLeaveInfo = JSON.parse(
            window.localStorage.getItem("editLeaveInfo")
        );
    ...
    methods: {
                ...
                if (this.vacationInfo) {
                    if (this.vacationInfo.thingNum > 0) {
                        this.typeOptions[0].label =
                            "事假（" + this.vacationInfo.thingNum + "小时）";
                    } else {
                    ...
                    if (this.vacationInfo.annualNum > 0) {
                        this.typeOptions[2].label =
                            "年假（" + this.vacationInfo.annualNum + "天）";
                    } else {
        ...
        getRestByUserId(userId) {
            getRestByUse(userId).then(response => {
                ...
                if (this.restInfo.length > 0) {
                    this.typeOptions[3].label =
                        "调休（" + this.restInfo.length + "天）";
                } else {
        ...
        disabledDate(time) {
            ...
            if (this.formData.type == 1 || this.formData.type == 3) {
                if (this.formData.start && !this.formData.end) {
                    return (
                        time.getTime() <
                            new Date(this.formData.start) -
                                1 * 24 * 3600 * 1000 ||
                        time.getTime() >
                            Date.now() +
                                this.vacationInfo.annualNum * 24 * 3600 * 1000
                    );
                }
            }
            ...
            if (this.formData.start && !this.formData.end) {
                return (
                    time.getTime() <
                        new Date(this.formData.start) - 1 * 24 * 3600 * 1000 ||
                    time.getTime() >
                        Date.parse(this.formData.start) +
                            (this.vacationInfo.annualNum - 1) * 24 * 3600 * 1000
                );
            }
            if (!this.formData.start && this.formData.end) {
                return (
                    time.getTime() <
                        Date.parse(this.formData.end) -
                            this.vacationInfo.annualNum * 24 * 3600 * 1000 ||
                    time.getTime() > new Date(this.formData.end)
                );
            }
        ...
        disabledDate2(time) {
            ...
            if (this.formData.start && !this.formData.end) {
                return (
                    ...
                    time.getTime() >
                        Date.parse(this.formData.start) +
                            (this.formData.time - 1) * 24 * 3600 * 1000
                );
            }
            if (!this.formData.start && this.formData.end) {
                return (
                    time.getTime() <
                        Date.parse(this.formData.end) -
                            this.formData.time * 24 * 3600 * 1000 ||
                    time.getTime() > new Date(this.formData.end)
                );
            }
            if (this.formData.start && this.formData.end) {
        ...
        disabledDate3(time) {
            switch (new Date().getDay()) {
                case 0:
                    return (
                        time.getTime() <
                            Date.now() - (0 + 7) * 24 * 3600 * 1000 ||
                        (time.getTime() >
                            Date.now() - (0 + 6) * 24 * 3600 * 1000 &&
                            time.getTime() <
                                Date.now() + (0 + 7) * 24 * 3600 * 1000) ||
                ...
                case 1:
                    return (
                        time.getTime() <
                            Date.now() - (1 + 2) * 24 * 3600 * 1000 ||
                        (time.getTime() >
                            Date.now() - (1 + 1) * 24 * 3600 * 1000 &&
                            time.getTime() <
                                Date.now() + (1 + 3) * 24 * 3600 * 1000) ||
        ...
        handleRemove(data) {
            ...
            this.formData.proveImg = this.formData.proveImg.replace(
                imgUrl + ",",
                ""
            );
        ...
        getStandardApplyInfo() {
            getStandardApplyInfoById(4).then(response => {
                if (response.data.data) {
                    ...
                    const leaveStandardApply = JSON.parse(
                        window.localStorage.getItem("leaveStandardApply")
                    );
